...
def experiment2():
    ...
    # 😈 DONE: Before we are sending the message back to Alice, we will replace Bob's public key...
    ...
    # 😈 DONE: derive the session key between us and Byron, too!
    ...
    # 😈 DONE: continue the communication to get the flag!
    j = receive(r)
    m = decrypt(alice_session_key, j['ciphertext'])
    print(f'Alice->Byron: {m}')
    j['ciphertext'] = encrypt(byron_session_key, m)
